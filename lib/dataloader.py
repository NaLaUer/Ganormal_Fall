...
class VideoDataset(Dataset):
    def __init__(self, args):
        ...
        self.resize_height = args.size
        self.resize_width = args.size
        ...
        file = os.path.join(self.label_dir, "train_normal.txt")
    ...
    def mkdir_begin(self):
        ...
        if not os.path.exists(self.label_dir):
            ...
            self.train_normal = os.path.join(
                self.label_dir, "train_normal.txt")

            train_bg = open(self.train_bg, 'a')
            train_normal = open(self.train_normal, 'a')
    ...
    def Video_to_Pic(self, file_dir, video_dir, dic):
        ...
        for video in os.listdir(video_dir):
            ...
            if video.split('.')[0] + '.txt' in dic:
                ...
                video_name = video.split('.')[0].replace(
                    ' ', '').replace('(', '').replace(')', '')

                save_address = os.path.join(
                    self.pic_dir, video_path_dir + video_name)
                os.mkdir(save_address)
                temp = save_address + " " + \
                    str(index_list[0]) + " " + str(index_list[1]) + " 1"
                ...
                temp = save_address + " " + \
                    str(0) + " " + str(index_list[0]) + " 0"

                with open(self.train_normal, "a") as f:
                    f.writelines(temp)
                    f.write('\n')
                f.close()
    ...
    def Read_Video(self, video_path, save_address, index_list):
        ...
        while (count < frame_count):
            ...
            if (frame_height != self.resize_height) or (
